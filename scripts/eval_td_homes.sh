...
TDHOMES_OUTPUT="${TDHOMES_OUTPUT:-evals/relpose-angular_VGGT}"
...
for TDHOMES_SHARD_INDEX in "${!TDHOMES_GPU_IDS[@]}"; do
  ...
  echo "Starting shard $TDHOMES_SHARD_INDEX/$TDHOMES_NUM_SHARDS on GPU $TDHOMES_GPU_ID"
  CUDA_VISIBLE_DEVICES="$TDHOMES_GPU_ID" "$TDHOMES_PYTHON" evaluation/eval_td_homes.py \
    --model "$TDHOMES_MODEL" \
...
if [[ "$TDHOMES_STATUS" -ne 0 ]]; then
  echo "At least one evaluation shard failed; inspect $TDHOMES_LOG_DIR" >&2
  exit "$TDHOMES_STATUS"
fi

"$TDHOMES_PYTHON" evaluation/eval_td_homes.py \
  --model "$TDHOMES_MODEL" \
  --output-dir "$TDHOMES_OUTPUT" \
  --num-shards "$TDHOMES_NUM_SHARDS" \
  --aggregate-only \
...
echo "Complete TD-HOMES results: $TDHOMES_OUTPUT"
